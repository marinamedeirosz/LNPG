...
def enviaValores(fldnome, spbxano, fldbanda, fldlanc):
    ...
    # salva no txt
    domain.salvaDados(nome, ano, banda, lanc)
...
def enviaArtista(txtfld, tree):
    ...
    # filtra albuns
    albunsFiltrados = domain.buscaAlbumPorArtista(artista)

    return albunsFiltrados


def enviaAno(spbx, radio, tree):
    # inicia valores
    ano = spbx.get()
    epoca = radio.get()
...
def janelaBuscaArtista():
    ...
    btnBusca = Button(
        winBusca,
        text="Buscar",
        command=lambda: incluiDadosFiltradosArtista(txtfldCantor, treeBusca),
    )
...
def janelaBuscaAno():
    ...
    ano = StringVar()
    ano.set("Anterior")
    rdoAnterior = Radiobutton(
        winBuscaAno, text="Anterior a", variable=ano, value="Anterior"
    )
    ...
    rdoDepois = Radiobutton(
        winBuscaAno, text="Depois de ", variable=ano, value="Depois"
    )
    ...
    btnBuscaAno = Button(
        winBuscaAno,
        text="Buscar",
        command=lambda: incluiDadosFiltradosAno(spbxAnoBusca, ano, treeBuscaAno),
    )
...
def janelaCadastro():
    ...
    btnBuscaArtista = Button(
        winCad, text="Pesquisar por artista", command=janelaBuscaArtista
    )
    ...
    btnSubmit = Button(
        winCad,
        text="Salvar",
        command=lambda: enviaValores(txtfldNome, spbxAno, txtfldBanda, v0),
    )
